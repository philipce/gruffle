...
This graph-based view of a workflow introduces _states_ (nodes in the graph), and _transitions_ (edges between nodes). A state is an immutable collection of data. It encapsulates all the necessary parameters to perform some work unit; essentially, it's a parameter object to some function. Those functions are called transitions. Transitions consume a single state and result in some number of successor states; this is where the bulk of the processing gets done. Thus, each unit of work in a workflow is a _state transition_—an origin state and a corresponding transition function. Whatever computation is required to advance through a workflow is encapsulated in its constituent state transitions.

This might just seem like a more complex way to say the same thing as our monolithic function. So, what value does this paradigm add? We mentioned the value proposition earlier: improved traceability, reliability, and scalability. We're now in a place to discuss why.
- **Traceability**: Because state transitions are explicitly defined, Gruffle can automatically trace details of execution for you: when a state was created, how long each transition took, what successor states resulted, etc. Most of the logging that a developer would need to manually insert into a monolithic function (and keep updated as the code evolves) can be achieved automatically. Logging granularity is easily controlled by creating finer- or coarser-grained states in your workflow.
